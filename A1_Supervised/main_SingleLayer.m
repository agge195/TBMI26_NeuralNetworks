...
% The training data
%ones(1,size(XBins{1},2));
%Ones1 = ones(1,size(XBins{1},2));

%m = size(XTrain, 1);
%XTrain = [XTrain, ones(m,1)];
%XTest = [XTest, ones(m,1)];

XTrain = [XTrain, ones(1, size(XTrain, 1))'];
XTest = [XTest, ones(1, size(XTest, 1))'];
...
learningRate  = 0.00000007; % Change this, your learning rate
%W0 = normrnd(0, 1, [size(DTrain, 1), size(XTrain, 1)]); % Change this, initialize your weight matrix W
W0 = rand(size(DBins{1}, 2), size(XTrain, 2))';
% Run training loop
tic;
